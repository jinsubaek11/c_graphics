...
vec3_t baraycentric_weights(vec2_t a, vec2_t b, vec2_t c, vec2_t p)
{
	...
	float alpha = (bc.x * bp.y - bc.y * bp.x) / area_triangle_abc;
	float beta = (ap.x * ac.y - ap.y * ac.x) / area_triangle_abc;
...
{
	vec2_t point_p = { x, y };
	vec3_t weights = baraycentric_weights(point_a, point_b, point_c, point_p);
...
)
{
	...
	if (y1 - y0 != 0)
	{
		for (int y = y0; y <= y1; y++)
		{
			...
			{
				draw_texel(x, y, texture, point_a, point_b, point_c,
					u0, v0, u1, v1, u2, v2);
			}
		}
	}
